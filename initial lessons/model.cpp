...
Model::Model(const std::string filename) : verts_(), uv_(), norms_(), facet_vrt_(), facet_nrm_(), facet_tex_(), diffusemap_(), specularmap_()
{
	std::ifstream in;
	...
	in.close();
	std::cout << "data extracted... file closed\n";
...
std::vector<int> Model::face_vertices() const
{
	return facet_vrt_;
}
